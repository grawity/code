...
$|            = 1;
my($p_name)   = $0 =~ m|/?([^/]+)$|;
my $p_version = build_version("20170101.0", '$Id: swaks 350 2017-01-02 02:51:31Z jetmore $');
my $p_usage   = "Usage: $p_name [--help|--version] (see --help for details)";
my $p_cp      = <<'EOM';
        Copyright (c) 2003-2008,2010-2017 John Jetmore <jj33@pobox.com>
...
sub sendmail {
	...
	# read the server's 220 banner. Also handle PROXY if requested
	do_smtp_connect() || do_smtp_quit(1, 21);
	...
	do_smtp_drop()     if ($G::drop_after_send eq 'helo'); # haaaack.  Need to use first-helo for this. Just quite here to prevent the mail from being delivered
	do_smtp_drop()     if ($G::drop_after eq 'helo');
	do_smtp_quit(1, 0) if ($G::quit_after eq 'helo');
...
sub do_smtp_xclient {
	...
	foreach my $attr (keys %{$G::xclient{attr}}) {
		...
		push(@parts, "$attr=" . join('', map { ($_ == 0x2b || $_ == 0x3d || $_ <= 0x20 || $_ >= 0xff)
		                                        ? sprintf("+%02X", $_)
		                                        : chr($_)
...
sub get_running_state {
	...
	if ($G::proxy{try}) {
		$s .= "yes\n";
		$s .= "  version     = $G::proxy{version}\n";
		if ($G::proxy{raw}) {
			$s .= "  raw string  = $G::proxy{raw}\n";
		} else {
			$s .= "  family      = $G::proxy{attr}{family}\n"
