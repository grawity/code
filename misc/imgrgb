...
my $sep1 = ";";
my $sep2 = ";";
my $promptheight = 3 + 1; # one extra line for the original command
my $fullwidth = 0;
my $debug = 0;
...
for my $row (0..$height-1) {
	...
	if ($row % 2) {
		...
		$buf .= "\e[0m (row $row/$height)" if $debug;
		$buf .= "\e[0m\e[K\n";
		@upper = ();
		@lower = ();
	}
}
# Print last half-line if needed. (If the image was shrunk in width, this can
# still result in an odd number of rows, despite 'LINES *= 2' making you think
# otherwise.)
if (@upper) {
	for my $i (0..@upper-1) {
		$buf .= "\e[38${sep1}2${sep2}${upper[$i]}m▀";
	}
	$buf .= "\e[0m (odd final)" if $debug;
	$buf .= "\e[0m\e[K\n";
}
print $buf;
