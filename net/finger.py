...
def finger(user, host, whois=False):
    ...
    for (family, type, proto, canonname, sockaddr) in gai:
        ...
        try:
            ...
            data = data.replace(b"\r\n", b"\n")
            if not data.strip():
                print("Error: Host %r returned nothing." % addr, file=sys.stderr)
                continue
            return cname or addr, data
        except OSError as e:
            print("Connection to %r failed: %r" % (addr, e), file=sys.stderr)
...
user, sep, host = args.target.rpartition("@")
addr, data = finger(user,
                    host if sep else "localhost",
                    args.long)
